...
bool Dse09sig::verify(CharmList & vpk, string & m, CharmList & sig)
{
    ...
    if ( ( (group.mul(group.pair(S1, group.exp(gbG2, s)), group.mul(group.pair(S2, group.exp(gba1, s1)), group.mul(group.pair(S3, group.exp(ga1, s1)), group.mul(group.pair(S4, group.exp(gba2, s2)), group.pair(S5, group.exp(ga2, s2))))))) == (group.mul(group.pair(S6, group.mul(group.exp(tau1, s1), group.exp(tau2, s2))), group.mul(group.pair(S7, group.mul(group.exp(tau1b, s1), group.mul(group.exp(tau2b, s2), group.mul(group.exp(wG2, -t), group.exp(hG2, t))))), group.mul(group.exp(group.pair(SK, group.exp(gG2, -t)), theta), group.exp(A, s2))))) ) )
    {
        return true;
    }
    else
