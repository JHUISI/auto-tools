...
void Bbibe04::keygen(CharmList & pk, CharmList & msk, string & id, CharmList & sk)
{
    ...
    r = group.random(ZR_t);
    K1 = group.mul(galpha, group.exp(group.mul(group.exp(uG2, ID), hG2), r));
    K2 = group.exp(gG2, r);
    sk.insert(0, K1);
    sk.insert(1, K2);
    return;
...
void Bbibe04::encrypt(CharmList & pk, GT & M, string & id, CharmList & ct)
{
    ...
    C1 = group.exp(gG1, s);
    C2 = group.exp(group.mul(group.exp(uG1, ID), hG1), s);
    ct.insert(0, C0);
    ct.insert(1, C1);
    ct.insert(2, C2);
    return;
...
void Bbibe04::decrypt(CharmList & pk, CharmList & sk, CharmList & ct, GT & M)
{
    ...
    K1 = sk[0].getG2();
    K2 = sk[1].getG2();
    R0 = group.mul(group.pair(C1, K1), group.exp(group.pair(C2, K2), -1));
    M = group.div(C0, R0);
    return;
}
