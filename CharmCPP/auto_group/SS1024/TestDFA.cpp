...
void Dfa12::keygen(CharmList & mpk, G1 & msk, CharmListInt & Q, CharmMetaListInt & T, CharmListInt & F, CharmList & sk)
{
    ...
    for (int i = 0; i < Tlen; i++)
    {
        ...
        K1[key] = group.mul(group.exp(D[t0], -1), group.exp(z, r));
        K2[key] = group.exp(g, r);
        K3[key] = group.mul(D[t1], group.exp(h[t2], r));
    }
    Flen = F.length();
    for (int i = 0; i < Flen; i++)
    {
        ...
        KendList1[x] = group.mul(msk, group.mul(D[x], group.exp(hend, rx)));
        KendList2[x] = group.exp(g, rx);
    }
    sk.insert(0, Kstart1);
    sk.insert(1, Kstart2);
...
void Dfa12::encrypt(CharmList & mpk, CharmListStr & w, GT & M, CharmList & ct)
{
    ...
    C1[0] = group.exp(g, s[0]);
    C2[0] = group.exp(hstart, s[0]);
    for (int i = 1; i < l+1; i++)
    {
