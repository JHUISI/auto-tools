...
void benchmarkSW(Sw05 & sw, ofstream & outfile0, ofstream & outfile1, ofstream & outfile2, int attributeCount, int iterationCount, CharmListStr & keygenResults, CharmListStr & transformResults, CharmListStr & decoutResults)
{
    ...
    CharmListZR blindingFactorDBlinded, blindingFactorDBlinded2;
	...
	for(int i = 0; i < iterationCount; i++) {
		benchK.start();
	    sw.extract(mk, w, pk, dParam, n, blindingFactorDBlinded2, bf0, skBlinded2);
		benchK.stop();
	...
	// BENCHMARK KEYGEN SETUP
    sw.extract(mk, w, pk, dParam, n, blindingFactorDBlinded, bf0, skBlinded);
	...
	for(int i = 0; i < iterationCount; i++) {
		...
		benchD.start();
		sw.decout(pk, dParam, transformOutputList, bf0, blindingFactorDBlinded, SKeys, SLen, transformOutputListForLoop, newM);
		benchD.stop();
