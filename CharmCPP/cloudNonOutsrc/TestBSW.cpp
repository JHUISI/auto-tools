...
void Bsw07::decrypt(CharmList & pk, CharmList & sk, CharmListStr & S, CharmList & ct, GT & M)
{
    ...
    for (int y = 0; y < Y; y++)
    {
        ...
        reservedVarName1 = group.exp(group.div(group.pair(Dj[yGetStringSuffix], Cr[yGetStringSuffix]), group.pair(Cpr[yGetStringSuffix], Djp[yGetStringSuffix])), coeff[yGetStringSuffix]);
        reservedVarName0 = group.mul(reservedVarName0, reservedVarName1);
    }
    A = reservedVarName0;
    result0 = group.pair(D, C);
