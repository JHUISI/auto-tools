...
void Ibeckrs09::setup(int n, int l, CharmList & mpk, CharmList & msk)
{
    ...
    for (int y = 0; y < n; y++)
    {
        z[y] = group.random(ZR_t);
        gl[y] = group.exp(g, z[y]);
        hl[y] = group.exp(h, z[y]);
    }
    v1 = group.exp(g, t1);
    v2 = group.exp(g, t2);
    v3 = group.exp(g, t3);
...
void Ibeckrs09::extract(CharmList & mpk, CharmList & msk, string & id, ZR & uf4, ZR & uf5, ZR & uf2, ZR & uf3, ZR & uf0, ZR & uf1, CharmList & skBlinded)
{
    ...
    d2Blinded = group.exp(d2, group.div(1, uf2));
...
void Ibeckrs09::transform(CharmList & skBlinded, CharmList & ct, CharmList & transformOutputList)
{
    ...
    c4 = ct[4].getG1();
    cpr = ct[5].getGT();
    transformOutputList.insert(5, cpr);
    transformOutputList.insert(0, group.pair(c0, d0Blinded));
    transformOutputList.insert(1, group.pair(c1, d1Blinded));
    transformOutputList.insert(2, group.pair(c2, d2Blinded));
    transformOutputList.insert(3, group.pair(c3, d3Blinded));
    transformOutputList.insert(4, group.pair(c4, d4Blinded));
    return;
}
...
void Ibeckrs09::decout(CharmList & transformOutputList, ZR & uf3, ZR & uf5, ZR & uf0, ZR & uf1, ZR & uf4, ZR & uf2, GT & M)
{
    ...
    cpr = transformOutputList[5].getGT();
    result = group.mul(group.mul(group.mul(group.mul(group.exp(transformOutputList[0].getGT(), uf0), group.exp(transformOutputList[1].getGT(), uf1)), group.exp(transformOutputList[2].getGT(), uf2)), group.exp(transformOutputList[3].getGT(), uf3)), group.exp(transformOutputList[4].getGT(), uf4));
