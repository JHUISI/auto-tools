...
void Bsw07::keygen(CharmList & pk, CharmList & mk, CharmListStr & S, ZR & uf0, ZR & bf0, CharmList & skBlinded)
{
    G1 gG1;
    G2 gG2;
    G1 hG1;
    ...
    DBlinded = group.exp(D, group.div(1, uf0));
    Y = S.length();
    for (int y = 0; y < Y; y++)
    {
    ...
    for (int y_var = 0; y_var < Dj_len; y_var++)
    {
        ...
        DjBlinded[y] = group.exp(Dj[y], group.div(1, bf0));
    }
    ...
    for (int y_var = 0; y_var < Djp_len; y_var++)
    {
        ...
        DjpBlinded[y] = group.exp(Djp[y], group.div(1, bf0));
    }
...
void Bsw07::transform(CharmList & pk, CharmList & skBlinded, CharmListStr & S, CharmList & ct, CharmList & transformOutputList)
{
    ...
    for (int y = 0; y < Y; y++)
    {
        ...
        FLrepVar1 = group.add(10, (5 * y));
        transformOutputListForLoop.insert(FLrepVar1, group.mul(group.pair(group.exp(Cr[yGetStringSuffix], coeff[yGetStringSuffix]), DjBlinded[yGetStringSuffix]), group.pair(group.exp(DjpBlinded[yGetStringSuffix], group.neg(coeff[yGetStringSuffix])), Cpr[yGetStringSuffix])));
        reservedVarName1 = transformOutputListForLoop[FLrepVar1].getGT().getGT();
        FLrepVar2 = group.add(11, (5 * y));
        transformOutputListForLoop.insert(FLrepVar2, group.mul(reservedVarName0, reservedVarName1));
        reservedVarName0 = transformOutputListForLoop[FLrepVar2].getGT().getGT();
    }
...
void Bsw07::decout(CharmList & pk, CharmListStr & S, CharmList & transformOutputList, ZR & bf0, ZR & uf0, GT & M)
{
    GT Ctl = group.init(GT_t);
    GT reservedVarName0 = group.init(GT_t);
    GT A = group.init(GT_t);
    ...
    result0 = group.exp(transformOutputList[2].getGT(), uf0);
