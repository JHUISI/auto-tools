...
void Bgw05::transform(CharmListInt & S, int i, int n, CharmList & Hdr, CharmList & pk, CharmList & skBlinded, CharmList & transformOutputList)
{
    ...
    sBlinded = skBlinded[0].getListG1();
    transformOutputList.insert(0, group.pair(glG1[i], Hdr2));
    numerator = transformOutputList[0].getGT();
    transformOutputList.insert(1, group.init(G1_t));
    dotProd2 = transformOutputList[1].getG1();
    lenS = S.length();
    for (int k = 0; k < lenS; k++)
    {
        ...
        if ( ( (j) != (i) ) )
        {
            ...
            transformOutputListForLoop.insert(FLrepVar2, group.mul(dotProd2, glG1[n+1-j+i]));
            dotProd2 = transformOutputListForLoop[FLrepVar2].getG1().getG1();
        }
    }
    transformOutputList.insert(2, group.pair(sBlinded[i], Hdr1));
    transformOutputList.insert(3, group.pair(dotProd2, Hdr1));
    return;
}
