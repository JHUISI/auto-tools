...
void Waters09::keygen(CharmList & pk, CharmList & msk, CharmListStr & S, ZR & bf0, CharmList & skBlinded)
{
    ...
    {
        z = S[y];
        Kl[z] = group.exp(group.hashListToG1(z), t);
    }
    ...
    for (int y_var = 0; y_var < Kl_len; y_var++)
    {
        ...
        KlBlinded[y] = group.exp(Kl[y], group.div(1, bf0));
    }
...
void Waters09::encrypt(CharmList & pk, GT & M, string & policy_str, CharmList & ct)
{
    ...
    for (int y = 0; y < Y; y++)
    {
        ...
        k = attrs[y];
        x = sh[y];
        Cn[k] = group.mul(group.exp(g1a, x), group.exp(group.hashListToG1(k), group.neg(r)));
        Dn[k] = group.exp(g2, r);
    }
...
void Waters09::transform(CharmList & pk, CharmList & skBlinded, CharmListStr & S, CharmList & ct, CharmList & transformOutputList)
{
    ...
    for (int y = 0; y < Y; y++)
    {
        ...
        FLrepVar1 = group.add(1000, (5 * y));
        transformOutputListForLoop.insert(FLrepVar1, group.mul(group.pair(group.exp(Cn[yGetStringSuffix], coeff[yGetStringSuffix]), LBlinded), group.pair(group.exp(KlBlinded[yGetStringSuffix], coeff[yGetStringSuffix]), Dn[yGetStringSuffix])));
        reservedVarName1 = transformOutputListForLoop[FLrepVar1].getGT().getGT();
        FLrepVar2 = group.add(1001, (5 * y));
