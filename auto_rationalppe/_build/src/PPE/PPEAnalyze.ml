...
let rec rule2loop sts pi k = 
    if (L.length pi.ppe_untrusted) <= k then 
        (false, EMPTY, EMPTY, pi, pi, false, k)
    else 
        let (success, isidentical, ckt, pi1, pi2, substitutesuccess) = rule2 sts pi k in
        if success then (success, isidentical, ckt, pi1, pi2, substitutesuccess, k)
...
let rec rule3loop sts pi k = 
    if (L.length pi.ppe_untrusted) <= k then 
        ...
        let (success, isidentical, ckt, pi1, pi2, substitutesuccess) = rule3 sts pi k in
        if success then (success, isidentical, ckt, pi1, pi2, substitutesuccess, k)
...
let rec rule4loop sts pi k = 
    if (L.length pi.ppe_untrusted) <= k then 
        ...
        let (success, isidentical, ckt, pi1, pi2, substitutesuccess) = rule4 sts pi k in
        if success then (success, isidentical, ckt, pi1, pi2, substitutesuccess, k)
...
and applyrule2loop sts problem k = 
    let (success, isidentical, ckt, pi1, pi2, substitutesuccess, k2) = rule2loop sts problem k in 
    if success then 
    (
        let (success1, ckt1) = analyze sts pi1 in
        if success1 then 
        (
            if substitutesuccess then 
            ( 
                ...
                (true, AND(AND(NOT(isidentical), ckt),ckt1))
        )
        else
            applyrule2loop sts problem (k2+1)
    )
...
and applyrule3loop sts problem k = 
    let (success, isidentical, ckt, pi1, pi2, substitutesuccess, k2) = rule3loop sts problem k in 
    if success then 
    (
        if isidentical = REJECT then
        (
            let (success1, ckt1) = analyze sts pi1 in
            if success1 then 
            (
...
and applyrule4loop sts problem k = 
    let (success, isidentical, ckt, pi1, pi2, substitutesuccess, k2) = rule4loop sts problem k in 
    if success then 
    (
        if isidentical = REJECT then
        (
...
let solve problem = 
    ...
    L.iter (fun f -> 
                    if RP.equal f.ge_rdenom RP.one then 
                        F.fprintf Format.std_formatter " %a in G%s, " RP.pp f.ge_rpoly (extract_group f.ge_group)
                    else 
                        F.fprintf Format.std_formatter " %a/%a in G%s, " RP.pp f.ge_rpoly RP.pp f.ge_rdenom (extract_group f.ge_group)
            ) added;
    L.iter (fun f -> 
                    if RP.equal f.ge_rdenom RP.one then 
                        F.fprintf Format.std_formatter "\n%a = %a in G%s" pp_fid f.ge_id RP.pp f.ge_rpoly (extract_group f.ge_group)
                    else
                        F.fprintf Format.std_formatter "\n%a = %a/%a in G%s" pp_fid f.ge_id RP.pp f.ge_rpoly RP.pp f.ge_rdenom (extract_group f.ge_group)    
            ) (problem.ppe_trusted@problem.ppe_untrusted@added);
    ...
    F.fprintf Format.std_formatter "\nComputed initial completion list. Took %fs" (Unix.gettimeofday() -. time);
    print_newline();
    ...
    F.fprintf Format.std_formatter "\nNormalized initial completion list. Took %fs" (Unix.gettimeofday() -. time);
    print_newline();
