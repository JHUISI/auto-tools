...
let print_problem problem =
    ...
    L.iter (fun f -> if (extract_group f.ge_group) <> problem.ppe_cgs.cgs_target then 
                    if RP.equal f.ge_rdenom RP.one then 
                        F.fprintf Format.std_formatter "%a = %a, " pp_fid f.ge_id  RP.pp f.ge_rpoly
                    else 
                        F.fprintf Format.std_formatter "%a = %a/%a, " pp_fid f.ge_id  RP.pp f.ge_rpoly RP.pp f.ge_rdenom
            ) problem.ppe_trusted;
    ...
    F.fprintf Format.std_formatter "\nUntrusted set in GT: ";
    L.iter (fun f -> (*F.fprintf Format.std_formatter " %a/%a " RP.pp f.ge_rpoly RP.pp f.ge_rdenom;
                     F.fprintf Format.std_formatter " in %s, " (extract_group f.ge_group);*)
                if ((extract_group f.ge_group) = problem.ppe_cgs.cgs_target) then
                    if RP.equal f.ge_rdenom RP.one then 
                        F.fprintf Format.std_formatter "%a = %a, " pp_fid f.ge_id  RP.pp f.ge_rpoly
                    else 
                        F.fprintf Format.std_formatter "%a = %a/%a, " pp_fid f.ge_id  RP.pp f.ge_rpoly RP.pp f.ge_rdenom
...
let multiply_zp poly zp = 
  ...
  (
    let output = L.fold_left (fun acc p -> acc@[{ge_rpoly = (RP.mult p.ge_rpoly poly.ge_rpoly); ge_rdenom = poly.ge_rdenom; ge_group = poly.ge_group; ge_id = Expzp(poly.ge_id, p.ge_rpoly)}]) [] zp in
  ...
  ( 
    ...
    L.iter (fun f -> F.fprintf Format.std_formatter " %a/%a, " RP.pp f.ge_rpoly RP.pp f.ge_rdenom) zp; 
    let rec mul l1 l2 num = (*multiply every element of l1 with every element in l2*)
      let l3 = L.fold_left (fun acc p -> acc@(L.map (fun a -> {ge_rpoly = (RP.mult p.ge_rpoly a.ge_rpoly); ge_rdenom = (RP.mult p.ge_rdenom a.ge_rdenom); ge_group = poly.ge_group; ge_id = Param(-1)}) l1)) [] l2 in
...
let add_non_duplicates lst1 lst2 = 
    L.fold_left (fun acc f -> if (L.exists (fun g -> (RP.equal f.ge_rpoly g.ge_rpoly) && (RP.equal f.ge_rdenom g.ge_rdenom) && (f.ge_group = g.ge_group)) acc) then acc
                              else f::acc) lst2 lst1

let extract_non_duplicates lst1 lst2 = 
    let (totallist, newlist) = L.fold_left (fun (totalacc, newacc) f -> if (L.exists (fun g -> (RP.equal f.ge_rpoly g.ge_rpoly) && (RP.equal f.ge_rdenom g.ge_rdenom) && (f.ge_group = g.ge_group)) totalacc) then (totalacc, newacc)
                              else (f::totalacc, f::newacc)) (lst2,[]) lst1 in
    newlist

let remove p lst = 
  L.filter (fun ele -> not (ele.ge_group = p.ge_group && RP.equal ele.ge_rpoly p.ge_rpoly && RP.equal ele.ge_rdenom p.ge_rdenom)) lst



let normalize lst = 
    let commondenom = L.fold_left (fun acc h -> (* F.fprintf Format.std_formatter "\n%a %a" RP.pp acc RP.pp h.ge_rdenom; *)
                                                RP.mult acc h.ge_rdenom) RP.one lst in 
    let indexedlst = L.mapi (fun index h -> (index,h)) lst in
    let normalizedlst = L.mapi (fun index f ->  {f with ge_rdenom = commondenom; 
                                            ge_rpoly = if RP.equal f.ge_rdenom RP.one then
                                                RP.mult f.ge_rpoly commondenom
                                            else 
                                                RP.mult f.ge_rpoly (L.fold_left (fun acc (index2,h) -> if index2 <> index then RP.mult acc h.ge_rdenom else acc) RP.one indexedlst); 
                                            } ) lst in
    (normalizedlst, commondenom)

let new_normalize added lst lstcommondnm = 
    let addedcommondenom = L.fold_left (fun acc h -> (* F.fprintf Format.std_formatter "\n%a %a" RP.pp acc RP.pp h.ge_rdenom; *)
                                                RP.mult acc h.ge_rdenom) RP.one added  in 
    let newcommondenom = RP.mult addedcommondenom lstcommondnm in 
    let normalizedlst = L.map (fun f -> {f with ge_rdenom = newcommondenom; ge_rpoly = RP.mult f.ge_rpoly addedcommondenom}) lst in 
    
    let indexedadded = L.mapi (fun index h -> (index,h)) added in
    let addednormalizedlst = L.mapi (fun index f ->  {f with ge_rdenom = newcommondenom; 
                                            ge_rpoly = if RP.equal f.ge_rdenom RP.one then
                                                RP.mult f.ge_rpoly addedcommondenom 
                                            else
                                                (RP.mult (RP.mult f.ge_rpoly lstcommondnm) (L.fold_left (fun acc (index2,h) -> if index2 <> index then RP.mult acc h.ge_rdenom else acc) RP.one indexedadded)); 
                                                } ) added in
    (normalizedlst@addednormalizedlst, newcommondenom)


let transferpoly problem k = 
    let poly = (L.nth problem.ppe_untrusted k) in 
    (* let counter = L.fold_left (fun max p -> if max < extract_fid p.ge_id then extract_fid p.ge_id else max) 0 problem.ppe_trusted@problem.ppe_untrusted in  *)
    let added = multiply_zp poly problem.ppe_zp in
    let added = poly::added in

    let (completion, normcompletion, commondnm) = if (extract_group poly.ge_group) = problem.ppe_cgs.cgs_target then
    (
        let (x, y) = new_normalize added problem.normcompletionGt problem.commondenom in
        (added@problem.completionGt, x, y)
    )
    else
    (
        ...
        let added = extract_non_duplicates completionlist problem.completionGt in 
        let (x, y) = new_normalize added problem.normcompletionGt problem.commondenom in
        (added@problem.completionGt, x, y)
    ) in 
    ...
    let problem = {problem with ppe_trusted = problem.ppe_trusted@added; ppe_untrusted = (remove poly problem.ppe_untrusted); completionGt = completion; normcompletionGt = normcompletion; commondenom = commondnm} in
