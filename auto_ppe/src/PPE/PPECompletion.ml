...
let apply_recipe recip0 : rpoly =
  let rec go recip : rpoly =
    match recip with
    | Param i        -> find_poly i
...
let apply_completion_ops cops cgid =
  ...
  let output = L.map (fun cop -> {ge_rpoly = apply_recipe cop; ge_id = cop; ge_group = Some cgid}) cops in
...
let completion_ops cgs cgid inp_gids =
  ...
  L.iter
    (fun (i,gid) -> add (ReP.var i) (Param i) gid) inp_gids;
...
let completion_ops2 cgs cgid inp_gids1 inp_gids2 =
  ...
  (* let first_non_one = Ss.cardinal cgs.cgs_gids in *)
  L.iter
    (fun (i,gid) -> add (ReP.var i) (Param i) gid) inp_gids1;
  L.iter
    (fun (i,gid) -> add2 (ReP.var i) (Param i) gid) inp_gids2;
...
let rec pp_recipe fmt c =
  ...
  | Exp(c, const) -> F.fprintf fmt "(%a)^%d" pp_recipe c const
